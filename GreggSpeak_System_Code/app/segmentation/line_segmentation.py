...
BINARY_CONSERVATIVE_LOW_PERCENTILE = 2.0
BINARY_CONSERVATIVE_PERCENTILE_MARGIN = 0.0
BINARY_MIN_THRESHOLD = 120
BINARY_MAX_THRESHOLD = 175
...
WORD_VERTICAL_PAD_FRACTION = 0.12
...
def binarize_image(enhanced_gray: np.ndarray) -> np.ndarray:
    """Create a conservative foreground-ink mask for segmentation only."""
    ...
    return remove_top_edge_artifacts(
        ink_mask,
...
def remove_border_artifacts(
    ...
    for label in range(1, component_count):
        ...
        if touches_border_zone and area <= max_area:
...
def drop_isolated_top_headers(
    ...
    isolated_top_text = (
        y < height * 0.22
        and width * 0.28 <= center_x <= width * 0.72
        and w < width * 0.55
        and h < height * 0.06
        and gap_to_next > max(40, int(height * 0.028))
    )
    return boxes[1:] if isolated_top_text else boxes
