...
BASE_V5_MODEL = MODELS_DIR / "best_all_words_v5_demo.keras"
BASE_V5_LABELS = MODELS_DIR / "labels_all_words_v5_demo.txt"

MODEL_OUT = MODELS_DIR / "greggspeak_all_words_v5_2.keras"
BEST_MODEL_OUT = MODELS_DIR / "best_all_words_v5_2.keras"
TFLITE_OUT = MODELS_DIR / "greggspeak_all_words_v5_2.tflite"
LABELS_OUT = MODELS_DIR / "labels_all_words_v5_2.txt"
HISTORY_OUT = REPORTS_DIR / "training_history_v5_2.csv"
METRICS_OUT = REPORTS_DIR / "final_metrics_v5_2.json"
SPLIT_SUMMARY_OUT = REPORTS_DIR / "dataset_split_summary_v5_2.json"
DEMO_PREDICTIONS_OUT = REPORTS_DIR / "demo_predictions_v5_2.csv"
RPI_PREDICTIONS_OUT = REPORTS_DIR / "rpi_predictions_v5_2.csv"
...
def parse_args():
    ...
    parser.add_argument("--base-model", type=Path, default=BASE_V5_MODEL)
    parser.add_argument("--base-labels", type=Path, default=BASE_V5_LABELS)
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Optional: start from the existing V5 weights instead of training like V5 demo.",
    )
...
def load_or_build_model(class_names, base_model_path: Path, base_labels_path: Path, from_v5: bool):
    if from_v5 and base_model_path.exists() and base_labels_path.exists():
        base_labels = load_label_order(base_labels_path)
        if base_labels == class_names:
            print(f"Loading V5 base model for V5.2 fine-tuning: {base_model_path}")
            model = tf.keras.models.load_model(base_model_path)
            backbone = set_backbone_mode(model, trainable_last_layers=None)
            return model, backbone
        print("V5 base model not loaded because label order changed.")

    print("Building a fresh V5.2 model.")
...
def main():
    ...
    dataset_labels = set(clean_files) | set(demo_files) | set(rpi_files)
    ...
    compile_model(model, 2e-4 if args.from_v5 else 1e-3)
    model.summary()
