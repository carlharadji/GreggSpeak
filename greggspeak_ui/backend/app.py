...
from db import (
    ...
    delete_page,
    edit_page,
    find_similar_criminal_case_numbers,
    generate_transcript,
    get_batch,
    get_all_batches,
    get_deleted_batches,
...
from network import get_access_urls, get_primary_access_url, print_access_urls
...
init_db()
DEFAULT_HOST = os.environ.get("GREGGSPEAK_WEB_HOST", "0.0.0.0")
try:
    DEFAULT_PORT = int(os.environ.get("GREGGSPEAK_WEB_PORT", "5000"))
except ValueError:
    DEFAULT_PORT = 5000
...
def default_tsn_fields(batch=None):
    ...
    if batch:
        ...
        if batch_id:
            saved_fields = get_tsn_metadata(batch_id)
            for key in values:
                if key in saved_fields:
                    values[key] = saved_fields.get(key) or values[key]
        if not values["certifier_name"]:
...
def format_tsn_export_fields(fields):
    data = dict(fields or {})
...
def settings_page():
    ...
    return render_template(
        ...
        database_path="greggspeak_ui/data/greggspeak.db",
    )
...
def user_batch_detail(batch_id):
    ...
    tsn_fields = default_tsn_fields(batch)
    return render_template(
        "user_batch.html",
        batch=batch,
        pages=pages_result.get("data", []),
        tsn_fields=tsn_fields,
        criminal_case_matches=find_similar_criminal_case_numbers(
            tsn_fields.get("criminal_case_no"),
            batch_id=batch_id,
        ),
    )


@app.route("/batch/<batch_id>/tsn-fields/save", methods=["POST"])
def save_tsn_fields(batch_id):
    batch_result = get_batch(batch_id)
    if not batch_result.get("success"):
        flash(batch_result.get("message", "Batch not found."), "error")
        return redirect(url_for("user_dashboard"))

    fields = get_tsn_form_data(batch_result.get("data") or {})
    matches = find_similar_criminal_case_numbers(
        fields.get("criminal_case_no"),
        batch_id=batch_id,
    )
    if matches:
        flash(criminal_case_conflict_message(matches[0], action="saved"), "error")
        return redirect(url_for("user_batch_detail", batch_id=batch_id))

    result = save_tsn_metadata(batch_id, fields)
    if not result.get("success"):
        flash(result.get("message", "Could not save TSN details."), "error")
        return redirect(url_for("user_batch_detail", batch_id=batch_id))

    flash("TSN details saved.", "success")
    return redirect(url_for("user_batch_detail", batch_id=batch_id))


@app.route("/api/tsn/criminal-case-check")
def api_tsn_criminal_case_check():
    case_no = request.args.get("case_no", "")
    batch_id = request.args.get("batch_id", "")
    matches = find_similar_criminal_case_numbers(case_no, batch_id=batch_id)
    return jsonify({"success": True, "matches": matches})


def criminal_case_conflict_message(match, action="saved"):
    case_no = match.get("criminal_case_no") or "matching case"
    title = match.get("title") or "another batch"
    return f"Similar case found: {case_no} ({title}). TSN details were not {action}."
...
def download_batch_pdf(batch_id):
    ...
    if request.method == "POST":
        raw_fields = get_tsn_form_data(batch)
        matches = find_similar_criminal_case_numbers(
            raw_fields.get("criminal_case_no"),
            batch_id=batch_id,
        )
        if matches:
            flash(criminal_case_conflict_message(matches[0], action="exported"), "error")
            return redirect(url_for("user_batch_detail", batch_id=batch_id))

        save_tsn_metadata(batch_id, raw_fields)
        fields = format_tsn_export_fields(raw_fields)
...
def admin_delete_page(page_id):
    return user_delete_page(page_id)
